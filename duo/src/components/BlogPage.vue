  ...
  <div id="container">
    <div id='top'>
      ...
      <div v-if="weatherInfo" class="weatherPanel"
        :style="{ position: 'absolute', right: '10px', top: '40px', zIndex: 100, background: 'white', padding: '10px', borderRadius: '5px', boxShadow: '0 2px 6px rgba(0,0,0,.3)', width: panelWidth, height: panelHeight }">
        ...
        <button @click="togglePanelSize">{{ panelState === 'expanded' ? '缩小窗体' : '恢复窗体' }}</button>
      </div>

      <!-- 展示旅游景点信息窗体 -->
      <div id="infobox" v-if="!listFlag">
        <ul>
          <li v-for="(feature, index) in features" :key="index" @click="highlightFeature(feature, index)">
...
export default {
  ...
  methods: {
    ...
    initMap() {
      AMapLoader.load({
        ...
        plugins: ['AMap.ToolBar', 'AMap.Scale', 'AMap.HawkEye', 'AMap.MapType', 'AMap.AutoComplete', 'AMap.ControlBar',
          'AMap.PlaceSearch', 'AMap.GeoJSON', 'AMap.Driving', 'AMap.StationSearch', 'AMap.Weather'],       // 需要使用的的插件列表，如比例尺'AMap.Scale'等
      }).then((AMap) => {
        ...
        this.infoWindow = new AMap.InfoWindow({ offset: new AMap.Pixel(0, -60) });
    ...
    addMarkerListeners(marker, feature) {
      marker.on('mouseover', () => {
        this.infoWindow.setContent(feature.properties.Name);
    ...
    highlightFeature(feature, index) {
      ...
      this.map.setCenter(selectedMarker.getPosition()); // 将视角转向选中的点
    ...
    togglePanelSize() {
      if (this.panelState === 'expanded') {
        ...
        this.panelWidth = '100px'; // 设置缩小后的宽度
        this.panelHeight = '50px'; // 设置缩小后的高度
      } else {
        // 恢复窗体
